...
let reg = execSync(`grep -o 'MJ[0-9]\\+' ./mjlist.txt`).toString().split("\n");
let table = execSync("cut ../tables/mji.00601.csv -d, -f2,3,4,6,8,30 | grep MJ0[12]").toString().split("\n").filter(v=>v).slice(0).map((row,i) => {
    let cell = row.split(",");
    ...
    console.log([jmj, "[[" + ref + "]]", ref ? "#a":"#ho", "onlynew" ].join("\t"));
